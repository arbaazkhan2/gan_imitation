...
def main(env_name, num_episodes, gamma, lam, kl_targ, batch_size, test):
    """ Main training loop
    ...
    aigym_path = os.path.join('~/Desktop/Videos', env_name, now)
    env = wrappers.Monitor(env, aigym_path, force=True)
    scaler = Scaler(obs_dim)
    val_func = NNValueFunction(obs_dim, test)
    policy = Policy(obs_dim, act_dim, kl_targ, test)
    ...
    #once training is done rn policy and get actions and states. 
    test_trajectories  = run_policy(env, policy, scaler, logger, episodes = 500)
    add_value(test_trajectories, val_func)  # add estimated values to episodesrf
    add_disc_sum_rew(test_trajectories, gamma)  # calculated discounted sum of Rs
    add_gae(test_trajectories, gamma, lam)
    ...
    np.save('expert_policies_states.npy', state_list)
    np.save('expert_policies_actions.npy', action_list)

        # save_dict = {'states': state_list, 'actions': action_list}
        # pickle.dump(save_dict, open('expert_policies.p', 'wb'))
